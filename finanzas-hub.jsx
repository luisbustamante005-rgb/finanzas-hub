...
function paidInsts(p){
  ...
  if(p.firstPaymentMonth){
    // firstPaymentMonth = month cuota was generated (e.g. mayo)
    // Payment is the following month (e.g. junio)
    // A cuota is "paid" once we're in or past its payment month
    const [fy,fm]=p.firstPaymentMonth.split("-").map(Number);
    // Payment month of cuota N = firstPaymentMonth + N months
    // How many cuotas have been paid = how many payment months have passed
    // We're in june (month 5, 0-indexed), first payment was june (fm=5, payment=fm+1=6)
    // months elapsed since first payment month (0=same month as generation, 1=payment month)
    const diff=(now.getFullYear()-fy)*12+(now.getMonth()+1-fm);
...
function instForMonth(purchase,yearMonth){
  ...
  if(purchase.firstPaymentMonth){
    const [fy,fm]=purchase.firstPaymentMonth.split("-").map(Number);
    // Cuota N is due in month firstPaymentMonth + N
    // So for yearMonth, which cuota number is it?
    // cuotaNum = (y-fy)*12 + (m-fm)
    const cuotaNum=(y-fy)*12+(m-fm);
    if(cuotaNum>=1&&cuotaNum<=purchase.installments) return {num:cuotaNum,amount:purchase.installmentAmt};
    return null;
  }
  const d=new Date(purchase.date+"T12:00");
  const diff=(y-d.getFullYear())*12+(m-(d.getMonth()+1));
...
function calcDistribution(data, mo){
  ...
  const directNec = data.expenses.filter(e=>e.date.startsWith(mo)&&!e.isWife&&getCatBucket(e.catId)==="necesidades").reduce((s,e)=>s+e.amount,0);
  const groceriesMo = data.groceries.filter(g=>g.date.startsWith(mo)).reduce((s,g)=>s+g.total,0);
  const instNec = data.purchases.filter(p=>!p.cancelled&&!p.isWife&&!p.isUber&&getCatBucket(p.catId||"varios")==="necesidades")
  ...
  const directEst = data.expenses.filter(e=>e.date.startsWith(mo)&&!e.isWife&&getCatBucket(e.catId)==="estilo").reduce((s,e)=>s+e.amount,0);
  const instEst = data.purchases.filter(p=>!p.cancelled&&!p.isWife&&!p.isUber&&getCatBucket(p.catId||"varios")==="estilo")
    .reduce((s,p)=>{const i=instForMonth(p,mo);return s+(i?i.amount:0);},0);
...
function Dashboard({data}){
  ...
  return(
    <div className="space-y-4">
      ...
      {(()=>{
        const mo=curMo();
        const uberIncome=data.incomes.filter(i=>i.type==="uber"&&(i.budgetMonth||i.date.slice(0,7))===mo).reduce((s,i)=>s+i.amount,0);
        const uberSpent=data.purchases.filter(p=>p.isUber&&!p.cancelled).reduce((s,p)=>{const inst=instForMonth(p,mo);return s+(inst?inst.amount:0);},0);
        const uberBal=uberIncome-uberSpent;
        if(uberIncome===0&&uberSpent===0) return null;
        return(
          <Card className="border border-orange-700/40">
            <p className="text-xs text-slate-400 font-medium mb-3 uppercase tracking-wider">🚗 Balance Uber</p>
            <div className="flex justify-between items-center mb-2">
              <span className={`text-2xl font-black ${uberBal>=0?"text-orange-400":"text-rose-400"}`}>{CLP(uberBal)}</span>
              <span className="text-xs text-slate-500">{uberBal>=0?"disponible":"déficit"}</span>
            </div>
            <div className="space-y-1 text-xs text-slate-400">
              <div className="flex justify-between"><span>+ Ingresos Uber</span><span className="text-orange-400">{CLP(uberIncome)}</span></div>
              <div className="flex justify-between"><span>− Gastos auto (cuotas mes)</span><span className="text-rose-400">{CLP(uberSpent)}</span></div>
            </div>
          </Card>
        );
      })()}
...
function Gastos({data,onChange}){
  ...
  const filtered=data.expenses.filter(e=>e.date.startsWith(filterMo)&&(filterBucket==="all"||e.bucket===filterBucket))
...
function Tarjetas({data,onChange}){
  ...
  const toggleUber=id=>onChange({...data,purchases:data.purchases.map(p=>p.id===id?{...p,isUber:!p.isUber}:p)});
          ...
          return(
            <Card key={p.id} className="!p-3">
              <div className="flex justify-between items-start mb-2">
                <div className="flex-1">
                  <p className="text-white font-semibold text-sm">{p.desc}</p>
                  <div className="flex gap-1.5 mt-1 flex-wrap">
                    {!p.isUber&&<BucketPill bucketId={p.bucket}/>}
                    {p.isUber&&<span className="text-xs px-2 py-0.5 rounded-full font-semibold bg-orange-900/40 text-orange-400 border border-orange-700/50">🚗 Uber</span>}
                    {p.isWife&&<Badge color="purple">👩 Esposa</Badge>}
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={()=>toggleUber(p.id)}
                    title={p.isUber?"Quitar de Uber":"Marcar como gasto Uber"}
                    className={`text-sm px-2 py-1 rounded-lg transition-all ${p.isUber?"bg-orange-900/40 text-orange-400":"bg-slate-700 text-slate-500 hover:text-orange-400"}`}>🚗</button>
                  <button onClick={()=>cancelPurchase(p.id)} className="text-slate-600 hover:text-rose-400 text-xs">×</button>
                </div>
              </div>
              ...
              <ProgressBar value={paid} max={p.installments} color={p.isUber?"bg-orange-500":"bg-blue-500"} height="h-1.5"/>
              {interest>0&&<p className="text-xs text-rose-400/70 mt-1">Interés total: {CLP(interest)}</p>}
              {p.isUber&&<p className="text-xs text-orange-400/60 mt-1">🚗 No afecta presupuesto del hogar</p>}
            </Card>
          );
        })}
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        const r=await window.storage.get("lhFinance_v3");
        if(r?.value){const parsed=JSON.parse(r.value);setData({...INIT,...parsed,savings:{...INIT.savings,...(parsed.savings||{})},cards:parsed.cards||[],purchases:parsed.purchases||[]});}
      }catch{}
      setLoaded(true);
    })();
  },[]);
  ...
  const onChange=useCallback(async nd=>{
    setData(nd);
    try{await window.storage.set("lhFinance_v3",JSON.stringify(nd));}catch{}
  },[]);
  ...
  if(!loaded)return <div style={{fontFamily:"'Sora',sans-serif"}} className="min-h-screen bg-slate-900 flex items-center justify-center"><p className="text-slate-400 text-sm">Cargando...</p></div>;
